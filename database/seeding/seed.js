...
// recipe image iterator
let imageNum = 0;

const seedDataBase = async () => {
  for (let i = 0; i < allRecipes.length; i++) {
    const recipesInDB = await Promise.all(
      allRecipes[i].map(async (el) => {
        // increment image number iterator
        imageNum++;
        ...
        const image = `../src/assets/recipe_images/recipe_image_${imageNum}`;
        mealType = mealType[0];
  ...
  const pantriesInDB = await Promise.all(
    pantryData.map(async (el) => {
